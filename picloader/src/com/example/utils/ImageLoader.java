...
public class ImageLoader
{
			...
			final boolean isFromNet,final long start_time,final int pos)
	{
		...
		//System.out.println(pos+":time1:"+start_time);
		if (mUIHandler == null)
		{
			mUIHandler = new Handler()
			{
				public void handleMessage(Message msg)
				{
					...
					long start_time=holder.start_time;
					int pos=holder.pos;
					// 将path与getTag存储路径进行比较
					if (imageview.getTag().toString().equals(path))
					{
						imageview.setImageBitmap(bm);
						long loadTime=System.currentTimeMillis()-start_time;
						Log.e("loadtime",pos+":"+start_time+":"+loadTime+"");
						LoadTime.allTime.add(loadTime);
					}
				};
			};
		}
		...
		if (bm != null)
		{
			refreashBitmap(path, imageView, bm,start_time,pos);
		} else
		{
			addTask(buildTask(path, imageView, isFromNet,start_time,pos));
		}
	...
	private void refreashBitmap(final String path, final ImageView imageView,
			Bitmap bm,long start_time,int pos)
	{
